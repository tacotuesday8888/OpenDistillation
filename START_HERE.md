...
Use the new local fact-miss diagnostic output to decide the next row-signal change before another GPU run. The diagnostic can separate invented numeric/time/identifier answers, same-chunk value confusion, known-value confusion, label echo, and answer-shape-without-fact misses. The disambiguation T4 smoke proved the rows were GPU-runnable, but the adapter still learned answer shape instead of exact values: 8 changed answers, 0/8 exact held-out hits, zero leakage, and zero unscored answers. Do not run another GPU smoke until a local data change has a concrete, testable reason to beat the previous best 1/8.
