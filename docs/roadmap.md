...
- Deterministic fact-ledger quality gate so safe fact extraction, train/eval leakage, and exact expected-term coverage are checked before optional training.
...
Current status: the bounded training engine, sample-fact/chunk-diverse comparison engine, runtime readiness helpers, manual smoke-test checklist, notebook entry points, fact-ledger quality gate, and anti-invention smoke preflight manifest exist. Local tests cover configuration, dataset formatting, runtime messages, comparison request behavior, reference-overlap scoring, fake base-vs-adapter generation, source-chunk diversity, held-out sample questions, adapter-disabled base generation, fact extraction including safe bullet/list facts, fact train/eval row building, train/eval leakage checks, expected-term checks, exact fact-hit scoring, exact-miss diagnostics, value-first fact-ledger targets, six-row label/value fact-ledger targets, direct held-out eval wording, notebook selection of fact-ledger rows for optional training, comparison metadata through row reordering, exact fact-hit report wording, SFT preview rows, same-chunk label/value disambiguation rows, known-values-only anti-invention rows, and the manifest contract used for the latest sample-notes T4 smoke. A clean GitHub-opened Colab T4 runtime completed the optional dependency install, sample-notes flow, mock-generated dataset, LoRA adapter training, and real before/after output once before the multi-question report existed. A later Tesla T4 quality smoke ran the 16-row dataset quality report, 3-step LoRA training, and 3-question before/after report; all trained-adapter answers were identical to the base answers. Follow-up diagnosis found the comparison path could compare against the adapter-enabled model on both sides; the local helper now uses PEFT's adapter-disabled inference path for base answers. A second T4 smoke after that fix made all three trained answers change, but they were generic or hallucinated rather than useful notes answers. The 2026-06-06 sample-fact CLI T4 run trained a 30-step adapter from 24 mock rows and compared four held-out sample-fact questions. It changed all four answers, but both base and trained answers hit 0/4 expected facts. The first 2026-06-08 fact-ledger CLI T4 run trained a 30-step adapter on 24 fact-ledger rows, changed all 8 held-out answers, and scored 0/8 exact expected facts for both base and trained answers. Follow-up local diagnosis found the supported SFT format was not the obvious problem, so the product-layer rows were changed to make exact values more salient and eval wording more direct. The revised value-first 2026-06-08 fact-ledger CLI T4 run also changed all 8 held-out answers and still scored base 0/8 and trained 0/8 exact expected facts. A later local audit fixed the reporting risk where expected terms could be attached by row position after comparison reordering; exact terms now travel with fact metadata. The 2026-06-15 six-row label/value CLI T4 run produced 48 fact-ledger train rows for the same 8 sample facts and improved exact held-out fact hits from 0/8 to 1/8, but still missed 7/8 facts. The 2026-06-16 same-chunk disambiguation CLI T4 run kept 48 rows and added 16 disambiguation rows, but changed all 8 answers and regressed to 0/8 exact held-out fact hits. Local diagnostics classified that failure as invented numeric/time/identifier values, and the anti-invention row signal replaced the risky swapped-value correction row with a known-values-only same-chunk row. The 2026-06-16 manifest-gated anti-invention T4 smoke changed all 8 answers and improved exact held-out fact hits from 0/8 to 2/8, beating the previous best 1/8, but it still missed 6/8 facts and every miss was an invented-value failure. Phase 3's useful behavior-change exit criterion is not met yet.
