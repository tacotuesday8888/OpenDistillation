...
The latest Colab T4 result proved that the pipeline can run, but it did not prove useful learning quality. The 2026-06-15 six-row label/value fact-ledger smoke trained on 48 fact-ledger rows and scored 8 held-out eval questions. The adapter changed all 8 answers and improved exact fact hits from 0/8 to 1/8, but still missed 7/8 checked facts. The current branch keeps the 48-row public shape and adds same-chunk label/value disambiguation rows because the last GPU run still confused nearby labels and values. The next evidence should be one bounded T4 smoke, not a broader product change.
