...
After the 2026-06-08 0/8 fact-ledger GPU result, the sidecar also records `value` and `row_style`. After the later label/value disambiguation work, contrast rows also record `contrast_label`, `contrast_value`, and `contrast_fact_id`. Comparison outputs also preserve internal `row_id` and `value` metadata so exact-miss diagnostics can explain wrong answers without changing the public JSONL schema. These are internal diagnostics only. They let OpenDistillation explain whether a row is an exact-value answer-only target, a label/value recall target, a same-chunk disambiguation target, a known-values-only anti-invention target, or a held-out direct-recall eval row, while the public JSONL schema stays unchanged. The local miss-context audit uses the same sidecar to show, for each missed fact, the exact training rows, row styles, same-chunk known values, known-values warning, and expected/invented value shape.
...
The current notebook also builds a deterministic fact-ledger report from the committed sample notes. The safe default path shows the number of extracted facts, six-row-per-fact train rows, held-out eval rows, train/eval leakage count, expected-term checks, label/value disambiguation coverage, anti-invention known-values coverage, and the first few fact cards before any optional training starts. A passing report means the local data/eval split is safer to test; it does not mean the small model has learned the facts.

The 2026-06-08 Colab T4 fact-ledger smokes used this same public schema for 24 fact-ledger train rows and 8 held-out eval rows. The schema and leakage checks passed in both the pre-fix run and the revised value-first run, but the trained adapter still scored 0/8 exact expected-term hits, the same as the base model. The 2026-06-15 six-row label/value T4 smoke used 48 rows and improved exact hits from 0/8 to 1/8, but still missed 7/8 facts. The 2026-06-16 same-chunk disambiguation T4 smoke kept the public schema and 48-row shape, added 16 same-chunk disambiguation rows, and regressed to 0/8 exact hits. The schema stayed stable; local diagnostics identified that failed trained-answer pattern as invented-value misses, and the anti-invention row signal kept the schema stable while replacing the risky swapped-value correction row with a known-values-only same-chunk row. The manifest-gated 2026-06-16 anti-invention T4 smoke checked the public schema, row hashes, held-out expected terms, and the 8 disambiguation plus 8 known-values-only row contract before GPU time. It improved exact hits from 0/8 to 2/8, but still missed 6/8 facts with invented values.
