...
- Local fact-miss diagnostics now sit on top of exact fact-hit scoring. They do not give credit for wrong answers; they explain misses as same-chunk value confusion, known-value confusion, invented numeric/time/identifier values, label echo, answer-shape-without-fact, or generic misses. The 2026-06-16 trained-answer fixture is covered locally and classifies all eight trained answers as invented-value misses.
- The current anti-invention row signal uses those diagnostics locally. It replaces the risky swapped-value correction row with a `known_values_only_label_value` row that only offers recorded same-note values and tells the model not to invent number/time/identifier/name/color substitutes. This is a local data-signal change, not new GPU evidence or a model-quality claim.
- The next anti-invention GPU evidence run must start with the internal smoke preflight manifest from `scripts/prepare_anti_invention_smoke.py`. The manifest is a local run contract, not a product CLI: it records source hashes, fact counts, row-style counts, zero-leakage checks, SFT preview rows, training config, held-out comparison questions, and the pass rule that trained exact fact hits must reach at least 2/8 to beat the previous best 1/8. Generated manifests should stay in `/tmp` or ignored output paths.
...
- Anti-invention T4 smoke preflight manifest generation is covered locally. The manifest pins the committed sample-notes contract at 8 facts, 48 train rows, 8 held-out eval rows, 8 disambiguation rows, 8 known-values-only rows, zero leakage, six SFT preview rows, row hashes, comparison-question hashes, and a failure rule that changed answers are not progress unless exact held-out fact hits beat the previous best 1/8.
...
- Whether a tiny Colab adapter run can produce useful note-grounded answers after the adapter-disabled comparison fix. Later sample-fact and fact-ledger smokes produced visible answer movement, but only reached a weak best of 1/8 exact held-out fact hits and then regressed to 0/8 on the disambiguation signal.
...
- Why the newly implemented local label/value disambiguation rows regressed to 0/8 exact held-out fact hits after the six-row run had reached 1/8.
