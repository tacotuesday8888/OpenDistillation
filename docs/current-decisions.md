...
- Fact-ledger train rows now use a six-row-per-fact label/value signal on this branch. The rows keep exact values prominent, repeat canonical `Label: value` bindings, preserve answer-only targets, and keep held-out eval rows in direct exact-recall wording. The internal sidecar records `row_style` and `value` for diagnosis. The public JSONL schema is unchanged.
...
- A local SFT preview now shows the exact prompt/completion pairs before training. On this branch the sample fact-ledger training signal has six rows per fact. The next GPU experiment should be one bounded T4 smoke with 8 facts, 48 fact-ledger train rows, 8 held-out eval rows, a 30-step `Qwen/Qwen2.5-0.5B-Instruct` LoRA adapter, and exact fact-hit scoring. If answers change but still miss the checked facts, that is a failure.
...
- Fact-ledger extraction, safe bullet/list fact extraction, train/eval row building, train/eval leakage checks, expected-term checks, and exact fact-hit scoring are covered by local tests. On this branch the committed sample notes produce 8 fact cards, 48 fact-ledger train rows, 8 held-out eval rows, 8/8 train coverage, 8/8 eval coverage, zero exact train/eval leaks, zero near-duplicate leaks, and zero missing expected terms in the safe notebook path.
...
- Whether the new six-row-per-fact label/value signal improves exact held-out fact hits in the next bounded T4 smoke.
