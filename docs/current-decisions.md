...
- Fact-ledger extraction, safe bullet/list fact extraction, train/eval row building, train/eval leakage checks, expected-term checks, exact fact-hit scoring, and exact-miss diagnostics are covered by local tests. In the current flow, the committed sample notes produce 8 fact cards, 48 fact-ledger train rows, 16 same-chunk disambiguation rows, 8 held-out eval rows, 8/8 train coverage, 8/8 eval coverage, zero exact train/eval leaks, zero near-duplicate leaks, and zero missing expected terms in the safe notebook path.
