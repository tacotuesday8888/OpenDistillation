...
- Why the revised value-first fact-ledger Colab T4 run still changed all 8 answers but hit 0/8 exact held-out facts.
- A product-level fix so exact fact-hit scoring carries expected terms by fact/question identity through comparison row reordering, not by row position in an external smoke script.
