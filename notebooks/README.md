...
- Optional chunk-diverse before/after quality report that skips when training is skipped.
...
As of 2026-06-03, the optional sample-notes training/comparison path has passed once from a clean GitHub-opened Colab T4 runtime, and the optional real-teacher path has passed one end-to-end T4 wiring check from sample notes through comparison. The first uploaded-notes rehearsal now has one `.txt` pass and one `.md` pass through validation, chunking, mock teacher rows, dataset save, training skipped, and comparison skipped. The deterministic dataset quality report is verified locally in the default notebook path and once in a Colab T4 quality smoke run after opt-in training. The first multi-question model-quality report also ran once in Colab, but all three trained-adapter answers were identical to the base answers. The comparison helper now fixes that report path by generating the base answer with the LoRA adapter disabled and by spreading comparison questions across source chunks first. A second Colab T4 quality smoke against that fix made all three adapter answers change, but the answers were still generic or hallucinated rather than useful note-grounded improvements. See `docs/colab-smoke-test-results.md` for exact package versions, adapter paths, memory notes, output-rendering caveats, historical upload-control blockers, and before/after output.
