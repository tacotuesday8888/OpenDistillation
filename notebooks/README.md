...
As of 2026-06-03, the optional sample-notes training/comparison path has passed once from a clean GitHub-opened Colab T4 runtime, and the optional real-teacher path has passed one end-to-end T4 wiring check from sample notes through comparison. Later uploaded-notes rehearsals have been blocked by browser automation before either `.txt` or `.md` could be verified. See `docs/colab-smoke-test-results.md` for exact package versions, adapter paths, memory notes, output-rendering caveats, upload-control blockers, and before/after output.
