...
As of 2026-06-03, the optional sample-notes training/comparison path has passed once from a clean GitHub-opened Colab T4 runtime, and the optional real-teacher path has passed one end-to-end T4 wiring check from sample notes through comparison. The first uploaded-notes rehearsal now has one `.txt` pass and one `.md` pass through validation, chunking, mock teacher rows, dataset save, training skipped, and comparison skipped. The deterministic dataset quality report is verified locally in the default notebook path. The current sample-notes default produces 4 chunks, 24 schema-valid fact-aware rows, 4/4 chunk coverage, zero duplicate or near-duplicate questions, zero answer-length warnings, and 4 held-out sample-fact questions while training and comparison stay skipped by default. The first multi-question model-quality report also ran once in Colab, but all three trained-adapter answers were identical to the base answers. The comparison helper now fixes that report path by generating the base answer with the LoRA adapter disabled and by spreading comparison questions across source chunks first. A second Colab T4 quality smoke against that fix made all three adapter answers change, but the answers were still generic or hallucinated rather than useful note-grounded improvements. The new 24-row / 30-step sample-fact quality smoke was prepared and pushed. The latest 2026-06-03 attempt did not reach notebook execution because Colab refused T4 GPU connection due to usage limits after CLI and browser fallback attempts. See `docs/colab-smoke-test-results.md` for exact package versions, adapter paths, memory notes, output-rendering caveats, historical upload-control blockers, GPU-limit evidence, and before/after output.
