...
- Optional before/after quality report that skips when training is skipped. When the fact-ledger gate passes, optional training uses fact-ledger train rows and comparison uses held-out fact-ledger eval questions with internal fact metadata for exact-hit scoring. The report also prints fact-miss diagnostics and a miss-context audit that links each wrong trained answer back to its training rows, row styles, same-chunk values, and known-values warning. The committed sample-notes fallback uses held-out fact questions; uploaded notes without fact-ledger rows use chunk-diverse generated questions.
