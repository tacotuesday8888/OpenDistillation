...
- A local fact-miss context audit connects wrong trained answers back to the exact training rows, row styles, same-chunk values, and known-values warning that taught the missed fact.
- A first-demo implementation plan.
- A manual Colab GPU smoke-test checklist.
- A smoke-test results file that records the first real Colab T4 blockers, a clean GitHub-opened T4 training/comparison pass, one real-teacher end-to-end T4 verification, the uploaded-notes rehearsal status, earlier multi-question Colab GPU quality smoke results, the 30-step sample-fact CLI T4 smoke, and the latest fact-ledger CLI T4 smokes.
...
- Evidence that the tiny adapter meaningfully improves answers on the fact-rich sample. The latest GPU evidence is the 2026-06-16 manifest-gated anti-invention fact-ledger Colab T4 smoke: 48 fact-ledger train rows, 8 disambiguation rows, 8 known-values-only anti-invention rows, 8 held-out eval questions, and exact fact hits improved from base 0/8 to trained 2/8. This passed the narrow predeclared rule by beating the previous best 1/8, but it is still weak because the trained adapter missed 6/8 checked facts and every miss was an invented-value failure.
...
The next work should use the local fact-miss context audit to change the data signal for the six remaining invented-value misses from the anti-invention T4 smoke. Do not broaden beyond the notes model, chase training knobs, or build export before the demo flow can show stronger note-grounded answers than 2/8 exact held-out hits. The future preflight gate now requires at least 3/8 exact held-out hits and at most 5/8 invented-value misses.
