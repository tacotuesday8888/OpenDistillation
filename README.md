...
- Evidence that the tiny adapter meaningfully improves answers on the fact-rich sample. The 2026-06-08 fact-ledger Colab T4 smoke trained on 24 fact-ledger rows and scored 8 held-out eval questions, but base and trained answers both hit 0/8 expected facts. The adapter changed all 8 answers without learning the checked facts.
- A verified post-fix GPU result for the revised value-first fact-ledger rows. The local product-layer rows now put exact values first, use direct exact-recall eval wording, and make the notebook train from fact-ledger rows when that gate passes, but no new adapter-quality claim exists until another bounded GPU smoke runs.
...
The next implementation work is to run one bounded value-first fact-ledger GPU smoke when non-interactive Colab access is available. Do not broaden beyond the notes model or build export before the demo flow can show useful note-grounded answers.
