...
- A six-row-per-fact label/value training signal on the current fact-ledger branch, so each sample fact gets more explicit `Label: value` supervision before optional training.
...
- Evidence that the tiny adapter meaningfully improves answers on the fact-rich sample. The latest GPU evidence is still the failed 2026-06-08 value-first fact-ledger Colab T4 smoke: 24 fact-ledger train rows, 8 held-out eval questions, and base/trained scores of 0/8 exact expected facts. The adapter changed all 8 answers without learning the checked facts. The new six-row local signal has not been GPU-tested yet.
