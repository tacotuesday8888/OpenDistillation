...
- Evidence that the tiny adapter meaningfully improves answers on the fact-rich sample. The 2026-06-08 value-first fact-ledger Colab T4 smoke trained on 24 fact-ledger rows and scored 8 held-out eval questions, but base and trained answers both hit 0/8 expected facts. The adapter changed all 8 answers without learning the checked facts.
...
The next implementation work is to strengthen the label-to-value learning signal locally before spending more GPU. The safest next data change is to add more per-fact training rows that repeat the canonical `Label: value` binding explicitly, then run the local quality gate and SFT preview before considering one bounded T4 rerun. Do not broaden beyond the notes model or build export before the demo flow can show useful note-grounded answers.
