...
- **Comparison:** optional bounded base-vs-adapter quality report after training; skipped by default and verified locally with fake model dependencies. For the committed sample notes, the strongest current report uses 8 held-out fact-ledger eval questions about concrete facts such as `Glass Harbor`, `copper-lantern-47`, `llama-harbor-alpha`, `4:17 PM`, and `ultramarine`. For uploaded notes, it falls back to generated questions from distinct source chunks first. The report uses PEFT's adapter-disabled inference path for the base answer. The latest fact-ledger Colab T4 smoke changed all 8 trained-adapter answers, but it still hit 0/8 expected facts and the answers were not useful note-grounded improvements.
...
The next implementation work is to diagnose why the 30-step fact-ledger smoke still hit 0/8 expected facts before spending more GPU. Start with local prompt/data/eval inspection, then make the smallest data or training-request change that improves the learning signal. Do not broaden beyond the notes model or build export before the demo flow can show useful note-grounded answers.
