...
- A six-row-per-fact label/value training signal with same-chunk disambiguation rows, so each sample fact gets explicit `Label: value` supervision and contrast against nearby labels before optional training.
...
- Evidence that the tiny adapter meaningfully improves answers on the fact-rich sample. The latest GPU evidence is the 2026-06-15 six-row label/value fact-ledger Colab T4 smoke: 48 fact-ledger train rows, 8 held-out eval questions, and exact fact hits improved from base 0/8 to trained 1/8. This is a bounded positive signal, but it is still weak because the adapter missed 7/8 checked facts. The newer local disambiguation rows have not been GPU-tested yet.
...
The next GPU experiment is exactly one bounded Colab T4 smoke using 8 facts, 48 fact-ledger train rows, 8 held-out eval rows, a 30-step `Qwen/Qwen2.5-0.5B-Instruct` LoRA adapter, and exact fact-hit scoring. Treat it as a failure if trained answers change but still miss the expected facts. Do not broaden beyond the notes model or build export before the demo flow can show useful note-grounded answers.
