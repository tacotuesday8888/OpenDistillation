...
class NotebookSkeletonTests(unittest.TestCase):
    ...
    def test_notebook_has_notes_flow_sections_and_no_saved_outputs(self):
        ...
        self.assertIn("diagnose_fact_misses", sources)
        ...
        self.assertIn("format_fact_miss_diagnostic_report", sources)
    ...
    def test_safe_notebook_path_builds_gpu_ready_fact_rows_without_training(self):
        ...
        self.assertEqual(fact_readiness_report.disambiguation_train_row_count, 8)
        self.assertEqual(fact_readiness_report.known_values_only_fact_coverage, 8)
        self.assertEqual(fact_readiness_report.known_values_only_train_row_count, 8)

        self.assertIn("SFT preview before any training:", output)
        self.assertIn("Exact SFT preview", output)
        self.assertIn("Label/value disambiguation rows: 8/8 contrastable facts, 8 total rows", output)
        self.assertIn("Anti-invention known-values rows: 8/8 contrastable facts, 8 total rows", output)
        self.assertIn("Verdict: local data split is structurally ready; model quality is still unproven", output)
